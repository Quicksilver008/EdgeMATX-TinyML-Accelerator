...
always @(alu_op, func3, sign_bit) begin
	case(alu_op)
		...
		//The register JAL register do not use the ALU , JALR uses ADD.
        	2'b11: begin
			case(func3)
			//  JAL is an unconditional jump: force zero_flag=1 so that
			//  ex_branch_taken = Branch(1) && zero_flag(1) = 1 always.
			//  ALU op 16 is reserved for exactly this: out=0, zero_flag=1.
			default: alu_select = 5'h10; // JAL (unconditional)
			endcase
		end	
		// DEFAULT UNKOWN
        	default: alu_select = 5'bxxxxx; 					// DEFAULT UNKOWN
